...
\begin{frame}{\CourseName\quad 第\LectureNum 回「\LectureTitle 」}
  ...
  \begin{exampleblock}{キーワード}
    \small \textbf{プロトコル} ／ \textbf{TCP/IP（Transmission Control Protocol / Internet Protocol）} ／ \textbf{bps（bits per second）} ／ \textbf{ファイアウォール} ／ \textbf{パケットフィルタリング方式} ／ \textbf{暗号化} ／ \textbf{SSL/TLS} ／ \textbf{デジタル署名} ／ \textbf{認証局（CA：Certificate Authority）}
  \end{exampleblock}
\end{frame}
...
\begin{frame}{演習}
  ...
  \begin{block}{演習1}
    回線交換方式とパケット交換方式の違いを説明せよ。また，災害時にSNSが使いやすい理由をこの違いと関連付けて述べよ
  \end{block}
  \vfill

  \begin{block}{演習2}
    TCP/IPの4階層それぞれの役割を説明せよ
  \end{block}
  \vfill
