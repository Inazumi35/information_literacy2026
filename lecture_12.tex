...
\begin{frame}{コミュニケーションの仕組み}
  ...
  \begin{alertblock}{演習}
    コミュニケーションにおける「符号化」と「解号化」を，具体例を挙げて説明せよ
  \end{alertblock}

  \begin{slidenote}
    「符号化」＝気持ちや考えを言葉・記号に変換すること。「解号化」＝受け取った記号を自分の経験で意味に変換すること。例：「ありがとう」という感謝の気持ち→言葉に変換（符号化）→相手が「感謝された」と理解（解号化）。符号化と解号化が一致しないと誤解が生じる（文化・方言の違いが原因になることも）。【演習解答例】例）「ありがとう」という感謝を言葉に変換（符号化）し，相手が感謝と受け取る（解号化）。
  \end{slidenote}
\end{frame}
