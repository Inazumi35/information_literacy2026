...
\begin{frame}{コミュニケーションの基本用語}
  \begin{description}[labelwidth=6em]
    \item[\kw{コミュニケーション}]
      人と人との間で情報を伝え合うこと。伝え手・受け手・メディアの三要素からなる
    \item[\kw{符号化}]
      伝えたい内容を記号や言葉などの信号に変換すること
    \item[\kw{解号化}]
      受け取った記号を経験や知識をもとに意味に戻すこと
    \item[\kw{メディアリテラシー}]
      メディアを通じて伝わる情報を主体的に読み解き，活用できる能力
  \end{description}
\end{frame}
...
\begin{frame}{アナログとデジタル}
  \begin{description}[labelwidth=6em]
    \item[\kw{アナログデータ}]
      連続的な量で表されるデータ。温度・音の波形など，自然界の多くがアナログ
    \item[\kw{デジタルデータ}]
...
\begin{frame}{課題・次回予告}
  \begin{importantbox}[課題（次回までに提出）]
    教科書 pp.34-39 を読み，10進数→2進数→16進数の変換を練習すること（5問以上）
  \end{importantbox}
