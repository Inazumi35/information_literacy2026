...
% Auto-generated from YAML (v3) — 後期14: データサイエンス・AI（7）：AIモデルの評価
...
\begin{frame}{推論とは}
  \begin{description}[labelwidth=6em]
    \item[\kw{推論（Inference）}]
      学習したモデルで新しいデータを予測すること
    \item[\kw{第12回（MNIST）での推論}]
      ①学習：60,000枚の手書き数字で学習 → ②推論：新しい手書き数字を見せる → ③予測：「これは7だ」と予測
    \item[\kw{評価の必要性}]
      モデルがどれくらい正確に推論できるかを数値で評価する。正解率だけでは不十分な場合がある
  \end{description}
\end{frame}
...
\begin{frame}{演習（後期後半まとめ）}
  ...
  \begin{block}{基本問題}
    \begin{itemize}
      \item 正解率・適合率・再現率の違いを説明せよ
      \item 「がん検診AI」を開発するとき，適合率と再現率のどちらを重視すべきか。理由も述べよ
      \item 過学習とはどのような状態か。なぜ問題なのか説明せよ
    \end{itemize}
  \end{block}
  ...
  \begin{exampleblock}{応用問題（余裕がある人）}
    \begin{itemize}
      \item 第8〜14回を振り返り，データサイエンスとAIの技術が機械工学にどのように応用できるか，具体的な場面を1つ考えて述べよ
    \end{itemize}
  \end{exampleblock}
\end{frame}
...
\begin{frame}{課題・次回予告}
  \begin{importantbox}[課題]
    次週は後期全範囲の復習。第8〜14回のハンドアウトと教科書pp.104-129を見直しておくこと
  \end{importantbox}
