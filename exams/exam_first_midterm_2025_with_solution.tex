...
\begin{itemize}
  ...
  \item マークシートのクラスは、M:01、E:02、I:03、C:04、A:05をそれぞれ記入・マークしてください。
  ...
  \item 解答は指定された解答欄に \textbf{正確に記入}してください。枠外の解答は採点対象にはなりません。
...
\subsection*{問1 学内システム活用とアカウント管理}
\begin{itemize}
  \item 1: \textbf{D} 統一アカウント - 学内の複数システムで共通使用できる
  \item 2: \textbf{B} 多要素認証 - セキュリティ向上のための認証方式
  \item 3: \textbf{B} OneDriveの主要な特徴はクラウドストレージと共同編集機能
  \item 4: \textbf{C} Teams - 双方向のやり取り（会議、チャット）に最適
  \item 5: \textbf{B} Forms - アンケート作成・集計に最適
\end{itemize}

\subsection*{問2 情報の特性と情報モラル}
\begin{itemize}
  \item 1: \textbf{A} 複製性 - 写真をネットに投稿したら、別サイトに転載された
  \item 2: \textbf{C} 特許権 - 技術的発明を保護する権利
  \item 3: \textbf{C} 意匠権 - 外観デザインを保護する権利
  \item 4: \textbf{C} フィッシング詐欺の典型例
  \item 5: \textbf{B} オプトイン - 事前同意の原則
\end{itemize}

\subsection*{問3 情報システムとクラウドコンピューティング}
\begin{itemize}
  \item 1: \textbf{C} 「処理」の例 - 入力された得点をもとに平均点を計算
  \item 2: \textbf{B} 「出力」の例 - 預金残高が画面に表示される
  \item 3: \textbf{C} PaaS - アプリ開発に必要な環境の提供
  \item 4: \textbf{B} スケーラビリティ - システム規模の柔軟な増減
  \item 5: \textbf{D} SaaS - ブラウザ上でGmailを使用
\end{itemize}

\subsection*{問4 PC設定とアカウント管理}
\begin{itemize}
  \item 1: \textbf{B} ローカルアカウント - オフライン使用可能
  \item 2: \textbf{B} Shift + F10 - Windows初期設定時のコマンドプロンプト起動
  \item 3: \textbf{B} BypassNRO - ネットワーク要求をバイパス
  \item 4: \textbf{C} ncpa.cpl - ネットワーク接続設定画面を開く
  \item 5: \textbf{C → D → E → B → A} - 正しい手順でネットワーク設定を行う
\end{itemize}

\subsection*{問5 Officeアプリケーションとクラウドサービス}
\begin{itemize}
  \item 1: \textbf{B} portal.office.com にアクセスし Microsoft アカウントでサインイン
  \item 2: \textbf{B} 有線LANでPCをインターネットに接続しているか確認
  \item 3: \textbf{B} .docx - Wordの標準ファイル形式
  \item 4: \textbf{C} OneDrive - Microsoft製クラウドストレージ
  \item 5: \textbf{C → B → D → A} - 正しいOfficeインストール手順
\end{itemize}
