...
\begin{frame}{OneDriveの役割と使い方}
  ...
  \begin{slidenote}
    OneDriveの「常時同期」と「バックアップとしてのコピー」の違いを理解しよう。常時同期はファイル操作のたびに通信が走るため，多人数が同時に使うと学内ネットが逼迫する。週1回程度，重要ファイルをコピーする運用が望ましい。
  \end{slidenote}
\end{frame}
...
\begin{frame}{Wordのローカル保存設定}
  ...
  \begin{slidenote}
    Wordも同様に「既定でコンピューターに保存する」設定を行う。PowerPointと同じ手順なので，自力でできるか確認しよう。
  \end{slidenote}
\end{frame}
...
\begin{frame}{ファイル管理の注意事項}
  ...
  \begin{slidenote}
    ファイル名に名前を入れる理由：提出後に誰のファイルか識別するため。保存場所の確認も習慣化しよう。
  \end{slidenote}
\end{frame}
...
\begin{frame}{演習：自己紹介を作ろう}
  ...
  \begin{slidenote}
    Word＝詳しい説明・文章向き，PowerPoint＝視覚的・発表向きという用途の違いに気づこう。両方作ることで，同じ内容でもソフトによって表現が変わることを体験できる。
  \end{slidenote}
\end{frame}
