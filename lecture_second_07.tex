...
\begin{frame}{基本用語}
  \begin{description}[labelwidth=6em]
    ...
    \item[\kw{構造化データ}]
      データベースで扱えるように整理されたデータ（例：表形式・数値データ）
    \item[\kw{非構造化データ}]
      画像・動画・音声・テキストなど，従来のデータベースで扱いにくいデータ
    \item[\kw{データベース（DB）}]
...
\begin{frame}{課題・次回予告}
  \begin{importantbox}[課題（次回までに提出）]
    教科書 pp.120-123 を復習すること。次週からデータサイエンスとAIの基礎を学ぶ。身近なAI活用の事例（画像認識・音声認識・推薦システム等）を1つ調べてくること
  \end{importantbox}
