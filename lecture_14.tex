...
\begin{frame}[shrink=8]{動画のデジタル表現}
  ...
  \begin{slidenote}
    \scriptsize インターレース：テレビ放送で使われてきた方式。プログレッシブ：PCモニタ・現代ディスプレイの主流。動画データ量の例：400×300，24bit，30fps，10秒 → $360{,}000\times30\times10\div1{,}048{,}576\approx102.9$ MB（無圧縮）。
  \end{slidenote}
\end{frame}
...
\begin{frame}[shrink=20]{非可逆圧縮}
  ...
  \begin{slidenote}
    \scriptsize JPEG：スマホ写真の標準形式（高圧縮）。保存を繰り返すたびに劣化する。MP3：CDの音楽データを約1/10に圧縮できる。MPEG：YouTubeやDVDで使われる。【演習解答】(1) $400\times300\times24\div8=360{,}000$ B，$360{,}000\times30\times10\div1{,}048{,}576\approx102.9$ MB。(2) $102.9\times0.05\approx5.1$ MB。
  \end{slidenote}
\end{frame}
