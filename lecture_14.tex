...
\begin{frame}[shrink=8]{動画のデジタル表現}
  ...
  \begin{alertblock}{演習}
    400×300画素，フルカラー（24bit），30fps，10秒の動画のデータ量（MB）を求めよ（1MB=1,048,576B）
  \end{alertblock}

  \begin{slidenote}
    \scriptsize インターレース：テレビ放送で使われてきた方式。プログレッシブ：PCモニタ・現代ディスプレイの主流。【演習解答】1枚 $400\times300\times24\div8=360{,}000$ B。動画 $360{,}000\times30\times10=108{,}000{,}000$ B $\div1{,}048{,}576\approx102.9$ MB（無圧縮）。
  \end{slidenote}
\end{frame}
...
\begin{frame}[shrink=20]{非可逆圧縮}
  ...
  \begin{slidenote}
    \scriptsize JPEG：スマホ写真の標準形式（高圧縮）。保存を繰り返すたびに劣化する。MP3：CDの音楽データを約1/10に圧縮できる。MPEG：YouTubeやDVDで使われる。圧縮率の例：1MBのファイルが200KBになった場合，$200\div1{,}024\approx0.20$（元の約20\%に圧縮）。【演習解答】可逆圧縮（ZIP/PNG）→元データに完全復元，テキスト・プログラム向き。非可逆圧縮（JPEG/MP3）→元に戻せないが高圧縮，写真・音楽向き。
  \end{slidenote}
\end{frame}
