...
\newcommand{\TextPages}{pp.128-131}
...
\begin{frame}{仮説検定の基礎}
  \begin{description}[labelwidth=6em]
    \item[\kw{仮説検定}]
      確率的な考え方に基づいて仮説が正しいかを統計的に判断する方法
    \item[\kw{全数調査}]
      対象全員を調べる調査
    \item[\kw{標本調査}]
      一部を取り出して調べる調査。標本から母集団の特徴を推定する
    \item[\kw{無作為抽出（ランダムサンプリング）}]
      偏りのない標本を得るための抽出方法
    \item[\kw{帰無仮説}]
      「差がない」「効果がない」など，否定したい仮説
    \item[\kw{有意水準}]
      帰無仮説を棄却する基準となる確率（例：5\%）。p値が有意水準を下回ったとき帰無仮説を棄却する
  \end{description}
\end{frame}
...
\begin{frame}{まとめ・チェックリスト}
  \begin{block}{自己チェック（できたら $\checkmark$ をつけよう）}
    \begin{itemize}
      \item[$\square$] 仮説検定（全数/標本調査・帰無仮説・有意水準・p値）の基礎を理解した
      \item[$\square$] 散布図と相関係数・線形回帰による予測の基本を学んだ
